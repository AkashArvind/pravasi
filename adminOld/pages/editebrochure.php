...
if(isset($_GET['ebrochure']))
{
    $ebrochureid=base64_decode(urldecode($_GET['ebrochure']));
    $table_name="ebrochures";
    $sql="select title,filename from ".$table_name." where id='".$ebrochureid."'";
    $q=mysql_query($sql);
    if($row=mysql_fetch_array($q))
    {
      ...
      $photo=$row['filename'];
      $dataorg="../../assets/images/ebrochure/".$photo;
    }
}
else
{
        ...
        <div class="col-md-12">             
            <h4>Edit E-brochure :
              <?php
                ...
                //$info = pathinfo($photo,PATHINFO_EXTENSION);
                if ($info["extension"] == "pdf")
                {?>
                  <a href="../../assets/images/ebrochure/<?= $photo ?>" target="_blank" class="text-center btn ntn-success btn-xs"><?=$photo?></a>
                 <?php
                }
                else
                {?>
                    <a href="<?= $dataorg?>" style="text-decoration: none;" data-title="<?=$photo?>" class="text-center btn ntn-success btn-xs" data-lightbox="images"><?=$photo?></a>
                  <?php

                }
                ?>
              <span style="float:right"><a href="ebrochures.php" class="btn btn-info btn-sm">Back</a></span></h4>
                ...
                <table class="table">
                  <tr class="info">
                    ...
                    <tr>
                        <td>Title</td>
                        <td colspan="3">
                          <input type="hidden" name="id" value="<?=$ebrochureid?>">
                    ...
                    <tr>  
                        <td colspan="4">
                          <button type="submit" name="editebrochure" class="btn btn-info btn-sm">Submit</button> 
                        </td>
                    </tr>
